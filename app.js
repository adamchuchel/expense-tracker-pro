...
/* ===== SETTLE UP SIDEBAR & FAB ===== */

// Initialize Sidebar
function initSettleSidebar() {
    const openBtn = document.getElementById('openSidebar');
    const closeBtn = document.getElementById('closeSidebar');
    const sidebar = document.getElementById('sidebar');
    const overlay = document.getElementById('sidebarOverlay');
    ...
    document.querySelectorAll('.sidebar-item[data-action]').forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            switch(action) {
                ...
                case 'about':
                    closeSidebar();
                    showToast('💎 Expense Tracker v5.0 - Settle Up Style');
                    break;
            }
        });
    });
    ...
    console.log('✅ Settle Up sidebar initialized');
}

// Update Sidebar User Info
function updateSettleSidebarUser() {
    const user = getCurrentUser();
    if (!user) return;
...
const originalInitApp = window.initializeApp;
if (originalInitApp) {
    window.initializeApp = async function() {
        await originalInitApp();
        initSettleSidebar();
        updateSettleSidebarUser();
    };
} else {
    // If initializeApp doesn't exist yet, run on DOMContentLoaded
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            initSettleSidebar();
            setTimeout(updateSettleSidebarUser, 1000);
        });
    } else {
        initSettleSidebar();
        setTimeout(updateSettleSidebarUser, 1000);
...
// Hook into loadOrganizationData
const origLoadOrg = window.loadOrganizationData;
if (origLoadOrg) {
    window.loadOrganizationData = async function() {
        await origLoadOrg();
        updateSettleSidebarUser();
...
console.log('✅ Settle Up scripts loaded');
